...
int main(int n, char* argv[])
{
	...
	if (cuda_err != cudaSuccess) {
		cerr << "Error al liberar memoria image en GPU\n";
		return -1;
	}
	cuda_err = cudaFree(ans_d);
	if (cuda_err != cudaSuccess) {
		cerr << "Error al liberar memoria ans en GPU\n";
		return -1;
	}
	cout << "Memoria liberada en device\n";
...
inline int _ConvertSMVer2Cores(int major, int minor)
{
	...
	cout << "MapSMtoCores for SM "<< major <<". "<< minor <<" is undefined. Default to use "<< nGpuArchCoresPerSM[index - 1].Cores << "Cores/SM" << endl;
